...
const Dashboard = {
  ...
  updateKPIs(data) {
    try {
      ...
      if (tc != null) {
        const e = byId('val-trade-count');
        if (e) e.textContent = Number(tc) === 0 ? 'No trades' : Number(tc) + ' trades completed';
      }
  ...
  async onStatusUpdate(data) {
    this.updateKPIs(data);
    if (Array.isArray(data.trades)) this.renderTrades(data.trades);
    else if (data.trades && data.trades.trades) this.renderTrades(data.trades.trades);
    if (data.positions) this.renderPositions(data.positions);
  ...
  renderPositions(positions) {
    ...
    c.innerHTML = pa.map(function(pos) {
      ...
      return '<div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:8px;margin-bottom:4px;padding-bottom:4px;border-bottom:1px solid var(--border-color)">' +
